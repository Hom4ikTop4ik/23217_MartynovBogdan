...
TBitArray* decompressFile(TBitArray* fileBitArray, TTree* tree)
{
    ...
    *ret = bitArrayEmpty();
    ...
    // if tree is only 1 leaf 
    if (fileBitArray->cnt == 0)
    {
        freeBitArray(ret);
        return NULL;
    }
...
void decompressArchive(TBitArray* archiveBitArray)
{
    ...
    for (int i = 0; i < cntOfFiles; i++)
    {
        ...
        // file was empty -> tree will empty too
        // (because I compress tree: if byte doesn't use in file I don't insert its into tree)
        // (0 bytes in file -> 0 bytes in tree)
        if (lenTree == 0)
            return;
        ...
        *file = bitArrayEmpty();
        ...
        // (1 "razniy" byte in file -> only 1 leaf in tree)
        // (1 leaf in tree -> compressed file is empty)
        // (so uncompressed file is leaf.value * lenOfUncompressedFile)
        if (decompressedFile == NULL)
        {
            int byte = tree->value;
            decompressedFile = malloc(sizeof(TBitArray));
            *decompressedFile = bitArrayEmpty();
            
            for (int i = 0; i < lenFileUncompressed; i++) {
                bitArrayPushByte(decompressedFile, byte);
            }
        }
